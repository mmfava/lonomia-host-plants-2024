...
df_lonomia <- rio::import("https://docs.google.com/spreadsheets/d/1PzJdUGWxhmMIk8-iGw9YqTbpHNNTPvCMUJOnHefPZD0/edit?usp=sharing") |>
  ...
  dplyr::rename(lat = `lat...`, long = `long...8`) |>
  dplyr::mutate(
    lat = ifelse(is.na(lat) & !is.na(`lat...18`), `lat...18`, lat),
    long = ifelse(is.na(long) & !is.na(`long...19`), `long...19`, long)
  ) 

df_lonomia2 <- df_lonomia |>
  ## * coluna que irá indicar quais coordenadas serão utlizadas na modelagem
  dplyr::mutate(
...
lonomia_name = "Lonomia achelous"
lonomia_kern = "achelous_native"
shape_type = 24
```
...
map_function <- function(lonomia_name, lonomia_kern, shape_type){ #kern_maps[[lonomia_kern]]
  ...
  scale_colour_manual(values = c("Buffer" = "#FF0000"), 
                        name = "🌍 Notificación de lonomismo") +
  ggplot2::geom_sf(data = dt_clean |> filter(species %in% lonomia_name), #lonomies
          aes(shape = "Lonomia"),
          size = 6, 
          fill = "#000000", 
          color = "#000000", 
